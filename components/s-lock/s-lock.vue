<template>
	<view class="s-lock">
		<canvas class="s-lock__warpper" @touchstart="touchstart" @touchmove="touchmove" @touchend="touchend" :disable-scroll="disableScroll" :style="{width: size + 'px', height: size + 'px'}" :canvas-id="canvasId"></canvas>
	</view>
</template>
	...
	export default {
		...
		mounted() {
			...
			this.defaultValue.forEach(value => {
				...
				const x = (this.squareData[value - 1].startSquare[0] + this.squareData[value - 1].endSquare[0]) / 2
				const y = (this.squareData[value - 1].startSquare[1] + this.squareData[value - 1].endSquare[1]) / 2
				this.order.push({
					x,
					y,
					value
		...
		methods: {
			...
			touchend() {
				...
				if(this.order.length > 0){
					// 提交答案
					this.$emit("complete", this.order.map(item => item.value))
				}
				this.clearData()
				this.renderCanvas()
			},
			...
			diffSquare({x, y}){
				...
				this.squareData.some((item, index) => {
					if(item.startSquare[0] <= x && x <= item.endSquare[0] && item.startSquare[1] <= y && y <= item.endSquare[1]){
			...
			initSquareData() {
				...
				for(let i = 0; i < 9; i++) {
					...
					// 一个方块区域直径加一个方块间隔
					const groupDistance = this.squreInterval + ( this.squareContainerRadius * 2 )
					// 左右两边剩余的间隔距离 ( 比例系数不满足下可以居中 )
					const surplusDistance = (this.size - ( this.squreInterval * 2 + this.squareContainerRadius * 2 * 3 )) / 2
					
					const startX = ((i % 3) * groupDistance) + (this.squareContainerRadius - this.squreTouchRadius) + surplusDistance
					const startY = ((row - 1) * groupDistance) + (this.squareContainerRadius - this.squreTouchRadius) + surplusDistance
					const endX = ((i % 3) * groupDistance) + (this.squareContainerRadius + this.squreTouchRadius) + surplusDistance
					const endY = ((row - 1) * groupDistance) + (this.squareContainerRadius + this.squreTouchRadius) + surplusDistance
			...
			renderCanvas(touche) {
				...
				this.squareData.forEach(item => {
					...
					if(item.active){
						...
						this.context.arc((item.startSquare[0] + item.endSquare[0]) / 2, (item.startSquare[1] + item.endSquare[1]) / 2, this.squreOutRadius + (+this.borderWidth), 0, 2 * Math.PI)
						this.context.fill()
						...
						this.context.beginPath()
						this.context.arc((item.startSquare[0] + item.endSquare[0]) / 2, (item.startSquare[1] + item.endSquare[1]) / 2, this.squreOutRadius, 0, 2 * Math.PI)
						this.context.fill()
						this.context.closePath()
						...
						// 设置填充颜色
						this.context.setFillStyle(this.squareColor)
					}
					
					
					this.context.beginPath()
					this.context.arc((item.startSquare[0] + item.endSquare[0]) / 2, (item.startSquare[1] + item.endSquare[1]) / 2, this.squareRadius, 0, 2 * Math.PI)
					this.context.fill()
					this.context.closePath()
				})
